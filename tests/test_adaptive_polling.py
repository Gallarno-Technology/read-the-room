...
async def test_poll_loop_uses_active_tier_when_playing(tmp_path, monkeypatch):
    ...
    async def fake_wait_for(coro, timeout):
        # Capture the timeout (which is next_sleep), then trigger stop_event.
        captured_sleeps.append(timeout)
        daemon.stop_event.set()
        # Cancel the coroutine we received so we don't leak it
    ...
    with patch("daemon.Path") as mock_path_cls, \
         patch("daemon.asyncio.wait_for", side_effect=fake_wait_for):
    ...
    daemon.stop_event.clear()
    assert captured_sleeps, "poll_loop must call asyncio.wait_for at the bottom of each iteration"
...
async def test_poll_loop_consumes_kick_file_and_short_circuits(tmp_path, monkeypatch):
    """A pre-existing poll_kick file is removed and next_sleep is forced to 0."""
    import daemon
    ...
    # When kick is consumed, sleep is either skipped entirely OR forced to ~0.
    # Implementation MAY skip wait_for entirely for next_sleep == 0; if it called
    # wait_for at all, the timeout must be 0.
    if captured_sleeps:
        assert captured_sleeps[0] == 0.0
