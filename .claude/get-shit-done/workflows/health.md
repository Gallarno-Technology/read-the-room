...
```bash
node "/home/cgallarno/Development/spotify-sentiment/.claude/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "/home/cgallarno/Development/spotify-sentiment/.claude/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="/home/cgallarno/Development/spotify-sentiment/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /home/cgallarno/Development/spotify-sentiment/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /home/cgallarno/Development/spotify-sentiment/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
